...
def create_symptoms(
    symptom: schema_symptons.SymptomsCreate,
    db: Session = Depends(get_db_session),
):
...
@routerMonitoring.get("/byPerId/{per_id}", response_model=List[schema_monitoring.MonitoringWithSymptom])
def get_monitoring_by_perId(
    per_id: int,
    db: Session = Depends(get_db_session),
):
...
def get_symtoms(
    ...
    return [schema_symptons.SymptomsInBD.model_validate(symptom.__dict__) for symptom in symptoms]
